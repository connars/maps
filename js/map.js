...
function initMap() {
  ...
  element3.addEventListener("click", function () {
    ...
    element4.classList.remove('active')
    element5.classList.remove('active')
    // map.setMapTypeId(google.maps.MapTypeId.STANDART);
    map.setMapTypeId(google.maps.MapTypeId.ROADMAP);

    // Убираем параметр стилей
    map.setOptions({ styles: null });
  });
  ...
  // Сохраняем текущий стиль карты
  currentStyles = map.get('styles') || [];

  // Получаем чекбокс и добавляем обработчик события change
  const checkbox = document.getElementById('toggle-labels');
  checkbox.addEventListener('change', function () {
    if (checkbox.checked) {
      // Отключаем лейблы, добавляя соответствующий стиль
      map.setOptions({
        styles: [
          ...currentStyles,
          {
      ...
      // Включаем лейблы, удаляя стиль отключения лейблов
      map.setOptions({
...
// function showProjectMap(action) {
 
//   slovakiaPolygon = new google.maps.Polygon({
...
//     // Add 10 random points inside the Slovakia polygon with info windows
//     // for (var i = 0; i < 10; i++) {
...
//     // }
//   } else if (action === 'hide') {
//     // Clear all markers and close info windows
//     slovakiaPolygon.setMap(null);
//     // infoWindow.close();
//     // Note: You should also keep track of all created markers and info windows to close/remove them properly
//   }
...
function showProjectMap(action) {
  ...
  document.querySelector('#pm-filters').classList.remove('hidden')
  if (action === 'show') {
    slovakiaPolygon.setMap(map);
 
    var infoWindow = new google.maps.InfoWindow({
      content:
        `<div class='zone_info pointer-events-none'>
          <p>Slovakia test</p>
        </div>`
    });

    google.maps.event.addListener(slovakiaPolygon, 'mousemove', function(event) {
      // Offset the position to the right of the cursor
      var offsetLatLng = google.maps.geometry.spherical.computeOffset(event.latLng, 10, 90);
      infoWindow.setPosition(offsetLatLng);
      infoWindow.open(map);
    });

    google.maps.event.addListener(slovakiaPolygon, 'mouseout', function() {
      infoWindow.close();
    });
  } else if (action === 'hide') {
    slovakiaPolygon.setMap(null);
...
function hideProjectMap() {
  ...
  document.querySelector('#pm-filters').classList.add('hidden')
}
